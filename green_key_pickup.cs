...
public class green_key_pickup : MonoBehaviour {
	...
	void Update () {
		...
		player = GameObject.Find ("player_main");
		if (SceneManager.GetActiveScene ().name == "balcony" && !GameObject.Find("game_stats").GetComponent<game_stats>().keepgreenkey) {
			if (close_enough && gameObject.GetComponent<SpriteRenderer> ().enabled && !placeininventory) {
				...
				gameObject.transform.Rotate (0, 0, -90.159f);
			...
			if ((placeininventory && !got_key_ui)) {
				GameObject.Find ("game_stats").GetComponent<game_stats> ().has_key_2 = true; //update overall stats saying player has key
				GameObject.Find ("game_stats").GetComponent<game_stats> ().score += 30;
				got_key_ui = true;
				GameObject.Find ("green_key_ui").GetComponent<SpriteRenderer> ().enabled = true;
			} else if (!placeininventory && !GameObject.Find ("game_stats").GetComponent<game_stats> ().keepgreenkey) {
				GameObject.Find ("game_stats").GetComponent<game_stats> ().has_key_2 = false;
				GameObject.Find ("green_key_ui").GetComponent<SpriteRenderer> ().enabled = false;
				if (GameObject.Find ("game_stats").GetComponent<game_stats> ().score >= 60) {
					GameObject.Find ("game_stats").GetComponent<game_stats> ().score -= 30;
				}
			} else {
				GameObject.Find ("green_key_ui").GetComponent<SpriteRenderer> ().enabled = true;
			}
		} else if (!GameObject.Find("game_stats").GetComponent<game_stats>().keepgreenkey) {
			gameObject.transform.position = Vector3.MoveTowards (gameObject.transform.position, GameObject.Find ("key_spot").transform.position, 10f * Time.deltaTime);
			gameObject.GetComponent<SpriteRenderer> ().enabled = true;
			GameObject.Find ("green_key_ui").GetComponent<SpriteRenderer> ().enabled = true;
		}

		if (GameObject.Find ("game_stats").GetComponent<game_stats> ().keepgreenkey) {
